...
// Geonix Master Orchestrator — runs all intelligence sources in parallel
...
export async function fullBriefing() {
  console.error('[Geonix] Starting intelligence sweep — 29 sources...');
  const start = Date.now();
  ...
  console.error(`[Geonix] Sweep complete in ${totalMs}ms — ${output.Geonix.sourcesOk}/${sources.length} sources returned data`);
  return output;
}
