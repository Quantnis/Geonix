...
export class DiscordAlerter {
      ...
      try {
        ...
        if (response) {
          ...
          if (response.length > 200) {
            await interaction.editReply({ embeds: [this._embed('Geonix', response, 0x00E5FF)] });
          } else {
  ...
  _buildAlertEmbed(evaluation, delta, tier) {
    ...
    if (embed.setFields) {
      ...
      embed.setFooter({ text: `Geonix Intelligence · ${new Date().toISOString().replace('T', ' ').substring(0, 19)} UTC` });
    } else {
      // Raw embed object for webhook fallback
      embed.fields = fields;
      embed.footer = { text: `Geonix Intelligence · ${new Date().toISOString().replace('T', ' ').substring(0, 19)} UTC` };
    }
